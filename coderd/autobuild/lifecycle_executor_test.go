...
func TestExecutorAutostopReminder(t *testing.T) {
	...
	// ActiveWorkspaceNotReminded: a workspace used within the lead window keeps
	// getting its deadline bumped, so no reminder is sent even though the tick
	// lands inside the window. This is the active-user guard, the exact
	// complement of the Sent subtest.
	t.Run("ActiveWorkspaceNotReminded", func(t *testing.T) {
		...
		// Mark the workspace as recently used: last_used_at within the lead
		// window of the tick makes currentTick - last_used_at <
		// time_til_autostop_notify, so the active-user guard suppresses the
