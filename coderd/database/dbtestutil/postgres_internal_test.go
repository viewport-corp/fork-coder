...
func TestIsTransientConnectError(t *testing.T) {
	...
	// Iterate the platform slice so Windows tests hit the real
	// WSAECONNREFUSED (10061), not the invented stdlib constant.
...
func TestRetryTransientConnect(t *testing.T) {
	...
	// Use the platform's first errno so the fixture is real on both
	// Linux (ECONNREFUSED) and Windows (WSAECONNREFUSED).
